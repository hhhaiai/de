...
class DeGPTClient:
    def __init__(self, debug: bool = False):
        ...
        self.MODEL_STATS = self.manager.dict(self.MODEL_STATS_initial)
        self.cached_models = self.manager.dict(self.cached_models_initial)
        self.lock = self.manager.Lock()
        self.models_found = self.manager.Value('b', False)  # 共享布尔标志
...
def worker_process(client_data, debug, user_prompt, model, result_queue):
    """
    子进程工作函数

    Args:
        client_data: dict, 包含共享数据
        debug: bool, 是否开启调试
    ...
    try:
        # 创建客户端实例，并加载共享数据
        client = DeGPTClient(debug=debug)

        # 载入共享数据
        client.MODEL_STATS = client.manager.dict(client_data['MODEL_STATS'])
        client.cached_models = client.manager.dict(client_data['cached_models'])
        client.models_found = client.manager.Value('b', client_data['models_found'])
...
if __name__ == '__main__':
    ...
    client_data = {
        ...
        'cached_models': client.get_cached_models(),
        'models_found': client.models_found.value
    }
    ...
    for i in range(5):  # 例如，启动5个进程
        ...
        p = multiprocessing.Process(
            ...
            args=(client_data, client.debug, user_prompt, "Pixtral-124B", result_queue),
            name=f"Worker-{i}"
        )
        p.start()
        processes.append(p)
